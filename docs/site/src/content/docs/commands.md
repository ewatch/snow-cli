...
persisted default, then JSON. See [`profile`](/commands/profile/) for
...
| [`api`](/commands/api/) | Send raw REST requests to arbitrary endpoints |
| [`graphql`](/commands/graphql/) | Submit a document to the optional Now GraphQL endpoint |
| [`script`](/commands/script/) | Run background scripts |
| [`snu`](/commands/snu/) | Drive the SN-Utils browser helper tab |
| [`codesearch`](/commands/codesearch/) | Search code and metadata on an instance |
...
echo '{"user_id":"import-user","email":"import-user@example.com"}' | snow-cli import-set load imp_user
```
